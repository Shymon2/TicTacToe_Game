...
static int evaluate_large_board(const vector<int>& board, int gridSize, int player) {
	...
	for (int i = 0; i < gridSize; ++i) {
		for (int j = 0; j < gridSize; ++j) {
			...
			if (consecutiveCount >= 2) score += 30000;
			if (enemyConsecutiveCount >= 2) score -= 30000;
			// end 2 threes check
...
random_device rd;
mt19937 rng(rd());
static Move find_best_move(vector<int>& board, int depth, int player) {
...
int main()
{
	...
	CROW_ROUTE(app, "/findmove").methods("POST"_method)([&](const crow::request& req) {
		...
		auto move = find_best_move(board, depth, PLAYER_COMP);
